...
class ListDataTable extends StatefulWidget {
  ListDataTable({
    ...
    _endpoint = '''
  query $endPointName$map_Variable{
          ...
          return FutureBuilder(
              ...
              builder: (BuildContext context, AsyncSnapshot<ValueNotifier<GraphQLClient>> snapshot) {
                  ...
                  return GraphQLProvider(
                    ...
                    child: Query(
                      ...
                      builder: (QueryResult result, {Refetch? refetch, FetchMore? fetchMore}) {
                        ...
                        networkData = repositories?.map((e) => toMapFunction(e)).toList() ?? [];
                ...
                if(snapshot.connectionState == ConnectionState.done){
                  return const Center(child: Text('Contact Administrator'),);
                }
    ...
    if(_searchKeyValue != null || _searchKeyValue != ''){
      UpdateTable.change.tableListData = networkData.where((e) {
       ...
       for (var element in e.keys) {
         if(e[element].toString().toLowerCase().contains(searchKey?.toLowerCase() ?? '')){
           list.add(e);
         }
       }
