...
class SelectionWidgetState<T> extends State<SelectionWidget<T>> {
  ...
  getNextPageFromOnline() async {
    if (widget.popupProps.isFilterOnline && widget.asyncItems != null) {
      _loadingNotifier.value = true;
      List<T> nextPageOnlineItems = [];
      nextPageOnlineItems = await widget.asyncItems!('', nextPage);
      if (!areEqual(nextPageOnlineItems, lastPageData)) {
        _cachedItems.addAll(nextPageOnlineItems);
        lastPageData = nextPageOnlineItems;
        nextPage++;
  ...
  Future<void> _manageItemsByFilter(String filter,
    ...
    if (widget.asyncItems != null &&
      ...
      try {
        ...
        onlineItems.addAll(await widget.asyncItems!(filter, null));
